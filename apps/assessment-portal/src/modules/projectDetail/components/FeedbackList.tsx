...
const FeedbackList: React.FC<{ projectId: string; dateValue: any }> = ({
	projectId,
	dateValue,
}) => {
	...
	const feedbackData = projectFeedbacks
		? projectFeedbacks?.slice(page * pageSize, pageSize * (page + 1))
		: [];
	...
	const handleSort = (selector: string, orderBy: string) => {
		setOrderBy({ selector: selector, order: orderBy });
		const tempArr =
			orderBy === "asc"
	...
	return (
		...
		>
			<Card bgColor="bg-white p-1 pt-3 mt-4">
				<FeedbackFilters dateValue={dateValue} projectId={projectId} />
				<div className="flex flex-wrap gap-1">
					<div className={`flex flex-col gap-[3px] bg-white w-full`}>
						...
						>
							...
							>
								{TableHeaders?.map((el: any, i: number) => (
									<>
										<TableHeader
											key={el.label}
										...
										>
											<div className="lg:flex flex-row items-center">
												<div className="flex basis-full items-center px-[10px] py-2 lg:bg-transparent bg-[#EFF2F7]">
													...
													<div className="ml-3 my-1 basis-1/5">
														<div
															className={` text-dark-200 text-[15px] flex items-center`}
														>
															{el.IsAnonymous ? (
